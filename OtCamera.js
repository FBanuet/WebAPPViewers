var scene,cameras,renderer,helper,frustrumSize;

init =()=>{
    scene = new THREE.Scene();
    const width = window.innerWidth;
    const heigth = window.innerHeight;
    const aspect = (width/heigth) * 0.5;
    cameras  = [];
    ...
    gui.add(params,'fov').min(20).max(80).step(1).onChange(value =>{
        cameras[0].fov = value;
    ...
    for(let x=-7; x<5; x+=1.5){
      for(let y=-9; y<5; y+=1.5){
